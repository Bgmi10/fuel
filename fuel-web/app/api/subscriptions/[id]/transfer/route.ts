...
export async function POST(
  ...
  try {
    // Apply your existing admin authentication check here.
    // This route must only be available to admins.
    ...
    const toMemberId = body.toMemberId?.trim();
    ...
    const result = await prisma.$transaction(
      async (tx) => {
        ...
        if (subscription.memberId === toMemberId) {
          throw new TransferError(
        ...
        if (receivingMember.status !== "ACTIVE") {
          throw new TransferError(
            "A blocked member cannot receive a membership."
          );
        }

        const fromMemberId = subscription.memberId;

        /*
         * Update only when the membership still belongs to the
         * original member. This prevents two admins transferring
         * the same membership at the same time.
         */
        ...
        if (subscription.invoiceId) {
          await tx.invoice.update({
            ...
            data: {
              memberId: receivingMember.id,
              memberName: receivingMember.name,
              memberPhone: receivingMember.phone,
              memberEmail: receivingMember.email,
            },
          });
    ...
    console.error("Membership transfer failed:", error);
